...
$(function() {
    ...
    if($('#txtCupomDesconto').val() ==  '') { 
        $('.table-boxCupom').css('display','none');  
    }
      
      if($('#Carrinho1txtCep').val() == '') { 
        $('.table-left').css('display','none');  
    }
      
    $(document).on('click', '.table-left-calc-2', function (event) {
        $('.table-left').show();
        event.preventDefault();
    });  
  
    $(document).on('click', '.table-boxCupom-calc-2', function (event) {
        $('.table-boxCupom').show();
        event.preventDefault();
    });   
  ...
  <script>
      $(document).ready(function() {
          ...
          $('.campoBt input').attr('onClick','teste()');
      });  
